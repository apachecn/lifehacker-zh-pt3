# 用这个搜索引擎看看你的护肤品里到底有什么

> 原文：<https://lifehacker.com/see-whats-actually-in-your-skincare-products-with-this-1826136459>

你知道你在皮肤上涂了什么吗？如果没有，你可以使用这个 [产品搜索引擎](https://incidecoder.com/) 来解码你的乳液、面霜和化妆品中有什么。



你可以在 INCIDecoder 上按名称搜索单独的成分和受欢迎的产品，它会列出所有的成分以及它们实际上是什么和它们有什么作用的描述。因为虽然我知道 Aqua 是什么，但我对 [PPG-26-Buteth-26](https://incidecoder.com/ingredients/ppg-26-buteth-26) 和 [乙基己基甘油](https://incidecoder.com/ingredients/ethylhexylglycerin) 不太熟悉。

这里有一个使用普普通通的自助餐的例子，我用它作为 PM 保湿剂:

它列出了所有的成分:

然后将它们分解，描述它们是否对你的皮肤有益，它们到底是什么，它们有什么作用，以及每种成分的刺激性。这里有一个例子:

类似的站点还有 [CosDNA](http://www.cosdna.com/) 和 [Skincarisma](https://www.skincarisma.com/) 。INCIDecoder 的没有 CosDNA 的那么宽泛，但我发现它更容易理解，如果你在网站上找不到，你可以创建一个帐户，并添加自己产品中的成分。如果你想花钱买一件产品，但又不确定你到底想要什么，这就非常有用了。

你也可以用它来找出过去刺激你皮肤的产品中有什么成分，这样你就可以在将来避免这些成分。
...
至于 Chrome OS，它已经自带了一个内置的文件管理器，但还有很大的改进空间。固体资源管理器具有光滑的材料设计灵感的外观和读取压缩或加密文件的能力。你也可以用不同的图标、颜色和主题来定制它的设计。

Solid Explorer 的真正卖点是它的两列布局，通过拖放可以很容易地移动文件。你可以使用该应用程序一次选择多个文件，并搜索任何你找不到的文件，在 Solid Explorer 的两周试用期结束后，你只需花费 2 美元(这是该应用程序的唯一缺点)。

这一更新(最初由 XDA 的开发者 发现)正在通过 [【谷歌 Play 商店】](https://play.google.com/store/apps/details?id=pl.solidexplorer2) 推出，但你可能需要再等几天才能接触到新功能。
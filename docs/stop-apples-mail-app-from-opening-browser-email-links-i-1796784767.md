# 只需一个简单的步骤，就可以阻止苹果的邮件应用打开浏览器邮件链接

> 原文：<https://lifehacker.com/stop-apples-mail-app-from-opening-browser-email-links-i-1796784767>

在 MacOS 中，在您使用的任何浏览器中点按电子邮件或“mailto”链接都会自动打开默认的电子邮件应用程序 Apple Mail。Stack Exchange 用户 Randy Marsh [建议快速更改设置](https://apple.stackexchange.com/questions/235598/how-can-i-disable-mail-app-when-i-click-on-an-email-address) 以阻止邮件意外启动。

Watch

要解决自动打开问题，请打开“邮件”应用程序，在菜单栏中选择“邮件”，然后前往“偏好设置”。您也可以使用“⌘+”快捷方式。在那里，你可以将默认的电子邮件应用程序更改为你喜欢的任何应用程序。

Marsh 指示选择位于您的 CoreServices 文件夹中的 Finder(HD→System→Library→core services→Finder . app)。点击链接将会把你切换到一个 Finder 窗口，而不是打开一个新的应用程序。

如果您经常使用网络，只需从应用程序文件夹中选择您的网络浏览器。从此以后，点击电子邮件只会让你停留在同一个浏览器窗口，实质上是禁用了自动打开功能。

让苹果邮件自动打开对于那些真正使用它的人来说可能很方便，但对于使用 Gmail 或 Outlook 等基于网络的邮件服务的人来说，在你点击每封邮件后打开一个不必要的应用程序是令人讨厌的，特别是当同样的事情可以通过复制和粘贴电子邮件链接来完成时。
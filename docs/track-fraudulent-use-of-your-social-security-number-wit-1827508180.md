# 跟踪欺诈使用您的社会安全号码与资本一的信用明智的工具

> 原文：<https://lifehacker.com/track-fraudulent-use-of-your-social-security-number-wit-1827508180>

Capital One 的 [Credit Wise](https://creditwise.capitalone.com/home) 是一款免费的信用跟踪工具，现在如果有人使用你的社会安全号码申请新的信用额度或开立另一个金融账户，它会提醒你。



任何人都可以使用这项服务，这项服务可以在线使用，也可以作为一个独立的应用程序用于和 [安卓](https://play.google.com/store/apps/details?id=com.capitalone.credittracker&hl=en_US) ，无论你是否拥有 Capital One 的帐户。它还跟踪你的信用报告，从益百利和 Transunion，并会通知你，如果有变化，并会提醒你，如果你的电子邮件地址或 SSN 被发现在黑暗的网络。

其他公司也提供类似的服务，但通常是收费的或向其成员提供的(例如，Discover [向所有人提供免费的 FICO 评分](https://www.nerdwallet.com/blog/credit-cards/discover-feature-alerts-social-security-number-risky-sites-dark-web/) ，但为持卡人保留 SSN 跟踪服务)。

Credit Wise 还提供了提高您的分数的步骤，并让您知道您在各种信用因素中的排名，如帐户历史和信贷利用率。

一个小问题:如果 Credit Wise 的应用程序像其他金融应用程序一样需要 pin 码才能访问，那就好了，这些应用程序也可以跟踪你的分数和报告。但是免费追踪用户的 SSN 是一个不错的奖励。

***更正:2018 年 7 月 13 日星期五:**这个故事的前一个版本错误地称 NerdWallet 的应用需要 pin 码才能访问。*
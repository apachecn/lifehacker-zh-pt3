# 这个 iOS 键盘应用程序会给你所有你需要的 Unicode 字符

> 原文：<https://lifehacker.com/this-ios-keyboard-app-will-give-you-all-the-unicode-cha-1822994495>

iOS: [Unicode 创建的表情符号](https://lifehacker.com/finding-the-right-emoji-can-be-hard-but-these-apps-mak-1819324984) 现在可能很流行，但当你需要在文档或完全诙谐的文本消息中访问更多晦涩的标点符号、重音字符和货币符号时，它们就没什么用了。不要像复制和粘贴动物一样复制和粘贴互联网搜索结果，使用[Unicode](https://jordanhipwell.com/UniChar/)，这是一个 iOS 键盘应用程序，它将 Unicode 的其余更传统的字符和符号(甚至盲文！)当你需要它们的时候，就在前面和中央。



UniChar 是一个第三方的 iOS 键盘，可以打开奇怪的 Unicode 字符的广阔世界。服务标志、版权标志、数学符号、技术符号，如苹果的“splat”图标或放射性指示器等，都可以在独立的应用程序和键盘插件中找到，只要你需要键入一些东西，你就可以使用它们。

你可以使用键盘将这些特殊字符添加到你的推文和文本中，但附带的独立应用程序提供了更多的功能。如果您发现自己在 web 的幕后工作，并且需要为特殊字符复制一些代码，您可以长按一个字符来复制符号、它的 Unicode 代码或在 web 上正确显示它所需的 HTML 代码。您还可以使用“按图形搜索”功能来查找您可能不记得其名称或类别的图标。它并不总是完美的，但它可能会唤起你足够的记忆，给你一个粗略的想法，你在寻找什么。

[Unichar 可以免费下载](https://itunes.apple.com/us/app/unichar-unicode-keyboard/id880811847?mt=8) ，但如果你想解锁应用中其余的 18 类角色，你需要进行 1.99 美元的应用内购买。
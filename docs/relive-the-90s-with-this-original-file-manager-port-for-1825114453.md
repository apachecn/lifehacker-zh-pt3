...
Windows 文件管理器绝对感觉像是另一个时代的遗物。虽然微软没有放弃 Windows 如何组织和呈现目录和文件的核心概念，但自 25 年前发布 Windows 3.0 以来，该公司无疑已经完善了这一过程。

WinFile 通过完美地展示现在古老的外观和感觉——包括图标的像素化艺术——在一个被遗忘的计算时代进行文件管理。也就是说，与基于文本的 [MS-DOS 命令](https://www.youtube.com/watch?v=MNwErTxfkUA) 或更丑陋的 DOS 外壳相比，文件管理器是当时急需的改进。微软的方法使移动、删除、复制和搜索文件变得超级容易，确保我们在 2018 年不再使用命令行(除非你真的真的想这么做)。